...
def process_input_variables_multidim(bios):
    ...
    indata1 = pd.read_csv(f'data/ADNI/{bios[0]}_data.csv', index_col=0)
    indata2 = pd.read_csv(f'data/ADNI/{bios[1]}_data.csv', index_col=0)
    indata3 = pd.read_csv(f'data/ADNI/{bios[2]}_data.csv', index_col=0)
...
if __name__ == "__main__":
    ...
    data = load_data_ADNI('data/ADNI/ADNIMERGE.csv')
    print()
    ...
    for biomarker in ['ABETA', 'TAU', 'PTAU']:
        ...
        biomarker_data.T.to_csv(f'data/ADNI/{biomarker}_data.csv')
